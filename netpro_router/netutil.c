...
int PrintEtherHeader(struct ether_header *eh,FILE *fp){
  ...
  fprintf(fp,"ether_type=%02x",htons(eh->ether_type));
...
arp.eh.ether_type=htons(ETHERTYPE_IP);

memset(buf,0,sizeof(buf));
p=buf;

memset(p,&arp.eh,sizeof(struct ether_header));p+=sizeof(struct ether_header);
memset(p,&arp.eh,sizeof(struct ether_arp));p+=sizeof(struct ether_arp);
total=p-buf;
